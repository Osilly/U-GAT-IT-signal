    ...
    def train(self):
        ...
        for step in range(start_step, self.num_epochs + 1):
            ...
            if step % 1000 == 0:
                ...
                params['disLA'] = self.disLA.state_dict()
                params['disLB'] = self.disLB.state_dict()
                torch.save(params, os.path.join('model_params_latest1.pt'))
...
if __name__ == '__main__':
    gan = Train(train_path='data/train', result_path='result/net1', signal_size=4096, num_epochs=500000, batch_size=4, light=True,
                 input_nc=1, output_nc=1, ch=64, n_blocks=6, lr=1e-4, weight_decay=1e-4, adv_weight=1, cycle_weight=10,
                 identity_weight=10, cam_weight=1000, decay_flag=True, device='cuda:1', resume=True)
    gan.dataload()
    gan.build_model()
    gan.define_loss()
    gan.define_optim()
